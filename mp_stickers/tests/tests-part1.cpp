...
TEST_CASE("Image invert works on hardcoded single pixels", "[weight=1][part=1]") {
  Image img;
  img.resize(1, 1);

  img.getPixel(0, 0).h = 45;
  img.getPixel(0, 0).s = 1;
  img.getPixel(0, 0).l = 0.55;
  
  img.invert();

  REQUIRE( img.getPixel(0, 0).h == 225 );
  REQUIRE( std::fabs(img.getPixel(0, 0).s - 0) < 1e-5);
  REQUIRE( std::fabs(img.getPixel(0, 0).l - 0.45) < 1e-5);

  img.getPixel(0, 0).h = 355;
  img.getPixel(0, 0).s = 0.3;
  img.getPixel(0, 0).l = 0.2;
  
  img.invert();

  REQUIRE( img.getPixel(0, 0).h == 175 );
  REQUIRE( std::fabs(img.getPixel(0, 0).s - 0.7) < 1e-5);
  REQUIRE( std::fabs(img.getPixel(0, 0).l - 0.8) < 1e-5);
}
